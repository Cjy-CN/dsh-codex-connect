...
The Host registers `llm-openai-codex` as the plugin-owned settings namespace and declares `OpenAI Codex` in the LLM configurable-provider directory. The browser binds that namespace through Harness's settings-scope transport and renders account, quota, Save/Discard capability controls in the existing Plugin configuration card. Revision-fenced field writes preserve unrelated settings. Committed changes reconcile search and image registrations live; the default-model and global-search namespaces are never written. An optional `contextWindow` rebuilds the next PiAiAdapter profile with model descriptors whose reported context capacity is overridden, while in-flight requests retain their captured profile; unsetting it restores the upstream catalog without a restart.
...
An explicit import action lets the Host read a bounded regular `$CODEX_HOME/auth.json` (default `~/.codex/auth.json`), rejecting symbolic links and group/world-readable POSIX modes. It accepts current flat `tokens` and legacy `tokens.chatgpt` OAuth shapes, decodes only the access JWT expiry and account claim, then copies the canonical access/refresh/account/expiry leaves into the plugin store. The source is never modified and no credential or absolute source path crosses the browser route. If the destination already exists, the unconfirmed request returns a conflict before reading the source; only the user's confirmed retry may replace it inside the destination file lock. This is one-time migration, not shared refresh-token synchronization.
