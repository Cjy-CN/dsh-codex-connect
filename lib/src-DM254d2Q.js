...
var OpenAICodexWebAuth = class {
	...
	async dispose() {
		this.cancellation?.abort(/* @__PURE__ */ new Error("OpenAI Codex plugin disposed"));
		await this.operation?.catch(() => void 0);
	}
	start() {
	...
	onEvent(event) {
		...
		if (new URL(event.url).protocol !== "https:") {
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.2.1";
...
async function fetchImage(source, maxBytes, signal) {
...
function viewImageTool(ctx) {
	return defineTool({
		...
		async execute(args, exec) {
			...
			let loaded;
			if (/^https?:\/\//iu.test(source)) loaded = await fetchImage(source, maxBytes, exec.signal);
			else {
...
function apply(ctx, config) {
	...
	if (config.enableImageTool === true) ctx.inject([
		"tools",
		"fs",
		"attachments"
...
//#endregion
export { openAICodexAuthPath as A, readOpenAICodexRateLimits as C, OPENAI_CODEX_AUTH_FILENAME as D, openAICodexAuthStatus as E, OPENAI_CODEX_PROVIDER as O, parseOpenAICodexUsage as S, logoutOpenAICodex as T, VIEW_IMAGE_TOOL_NAME as _, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as a, openAICodexConflictMessage as b, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as c, OPENAI_CODEX_SEARCH_URL as d, OpenAICodexSearchProvider as f, recordOpenAICodexSearchRequest as g, installOpenAICodexSearchEvent as h, name as i, OpenAICodexCredentialStore as k, OPENAI_CODEX_BASE_URL as l, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as m, apply as n, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as o, mapOpenAICodexSearchResponse as p, inject as r, DEFAULT_OPENAI_CODEX_SEARCH_MODE as s, Config as t, OPENAI_CODEX_SEARCH_PROVIDER as u, assertNoOpenAICodexProviderConflict as v, loginOpenAICodex as w, OPENAI_CODEX_USAGE_URL as x, diagnoseOpenAICodex as y };
